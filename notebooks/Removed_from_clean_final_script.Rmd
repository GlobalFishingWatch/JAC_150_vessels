...
plot_points <- 
  voi_150402944 %>%
  filter(!is.na(lat), !is.na(implied_speed_knots))

# 30/11/2021 South Atantic class B
# 29/07/2021 China

max(plot_points$meters_to_prev)/1800
max(plot_points$implied_speed_knots)
write.csv(plot_points, here::here("data", ".", "plot_points_150402944.csv"))

voi_150402944_a <- voi_150402944 %>% filter(between(timestamp, date_start, date_end), AIS_class == 'A') 
voi_150402944_b <- voi_150402944 %>% filter(between(timestamp, date_start, date_end), AIS_class == 'B') 

# make st object of identity points
recenter_150402944_p1 <- voi_150402944_a %>%
...
recenter_150402944_p1_trans <- st_transform(recenter_150402944_p1, crs=best_proj)

recenter_150402944_p2 <- voi_150402944_b %>%
  dplyr::select(timestamp, lat, lon) %>%
  sf::st_as_sf(., 
               coords = c("lon", "lat"), 
               crs = 4326) %>%
...
recenter_150402944_p2_trans <- st_transform(recenter_150402944_p2, crs=best_proj)
  # geom_sf(data = recenter_150402944_p1_trans, colour=gfw_palettes$primary[1]) +
  # geom_sf(data = recenter_150402944_p2_trans, colour=gfw_palettes$primary[2]) +
