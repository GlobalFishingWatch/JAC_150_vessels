...
working_df %>% write.csv(here::here("data", ".", paste0("150_voi_working_df",str_replace_all(Sys.Date(), '-','_'),".csv")))
...
voi_identity_segs <- fishwatchr::gfw_query(query = glue::glue(voi_identity_q,
                                               voi = paste0("('",paste(unique(working_df$ssvid), collapse="','"), "')")),
                                 run_query = TRUE,
                                 con = con)$data

View(voi_identity_segs)
