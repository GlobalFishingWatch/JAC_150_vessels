...
```{r, echo=F}
# set bouding box up 
# bounding <- transform_box(xlim = c(-55, 100),
#                           ylim = c(-90, 90),
...
# open up shipname by one day to be visible on plot 
time_plot_df2$last_trans[time_plot_df2$shipname == 'LURONGYUANYU 581']
# "2021-02-17 18:06:00 GMT"
time_plot_df2$last_trans[time_plot_df2$shipname == 'LURONGYUANYU 581'] <- "2021-02-18 18:06:00 GMT"
time_plot_df3$last_trans[time_plot_df3$shipname == 'LURONGYUANYU 581'] <- "2021-02-18 18:06:00 GMT"
...
# save outputs 
ggsave(here::here("outputs", "figures", "LRYY715_timeline_map_v3.png"), width = 10, height = 7)
...
seg_df <- seg_df %>% filter(shipname != 'LURONGYUANYU 581')

# make the recentered plot df 
recenter_lryy715_v2 <- seg_df %>%
  filter(!is.na(lat)) %>%
...
# save output in outputs directory 
ggsave(here::here("outputs", "figures", "LRYY715_name_map_v2.png"), width = 10, height = 7)
...
```
```{r, echo=F, warning=F}
daily_identity$region <- 'Other'
daily_identity$region[between(daily_identity$lon, -120, -72) & between(daily_identity$lat, -60, 0)] <- 'Pacific'
daily_identity$region[between(daily_identity$lon, -61, -50) & between(daily_identity$lat, -49, -37)] <- 'Atlantic'
...
time_plot_df <- daily_identity %>% group_by(region, shipname) %>% summarise(
  first_trans = min(day), 
  last_trans = max(day),
  n=n()
)
...
working_df$class <- NA
working_df$class[working_df$type %in% c('AIS.1', 'AIS.3')] <- 'A'
...
filter(working_df, ssvid %in% voi_that_transmit) %>% group_by(ssvid, class) %>% summarise(n=n())
...
# drop extra columns 
annex2_m <- annex2 %>% dplyr::select(-Transmissions, -Name.count)
...
#                                  con = con)$data
# 
# data_a_b %>% saveRDS(here::here("data", ".", "top_6_ab_2023.rds"))

data_a_b <- read_rds(here::here("data", ".", "top_6_ab_2023.rds"))

table(data_a_b$AIS_class, data_a_b$ssvid)
...
# seperate class A and class B
data_150400453_a <- data_a_b %>% filter(ssvid == '150400453', AIS_class == 'A')
data_150400453_b <- data_a_b %>% filter(ssvid == '150400453', AIS_class == 'B')
write.csv(data_150400453_a, here::here("data", ".", "150400453_a_2023.csv"))
write.csv(data_150400453_a, here::here("data", ".", "150400453_b_2023.csv"))
