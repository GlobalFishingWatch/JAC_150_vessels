...
ggplot() +
  ...
  geom_gfw_eez(theme = 'light', center = new_center, alpha = 0.05) +
  geom_sf(
...
# identify group change points based on segment differences
j=1
i=1
df$grp <- NA
for (i in 2:nrow(df)){
...
st_as_sfc(daily_identity$avg_lat_lon, crs = st_crs(4326)) %>% fishwatchr::recenter_sf(., center = new_center)
...
tab_df<- gear_df %>% 
  group_by(ssvid, region, speed_bin) %>% 
    summarise(transmissions=n()) %>% 
      left_join(gear_df %>% group_by(ssvid, region) %>% summarise(region_ttl=n()))

tab_df$prop <- tab_df$transmissions/tab_df$region_ttl 
